...
public class GUI extends JFrame {
    ...
    private Font font;
    private Customer c = new Customer();//classe customer, che identifica i record, in questo caso è una specie di buffer
    ...
    public GUI(Database db) {
        super();
        table = new JTable(db.defaultTableModel);//istanzio la tabella
        ...
        ListenForMouse mouseListener = new ListenForMouse();//ListenforMouse è una classe. e serve per ascoltare eventi da mouse
    ...
    private class ListenForAction implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            //è il metodo che viene lanciato quando clicco sul pulsante
            ...
            if (e.getSource() == addCustomer) { // If the user clicks Add MainCRUD, add the information into the database
                ...
                if (!c.getState().matches("[A-Za-z]{2}")) {//serve per verificare che corrisponde ad un certo elemento. in questo caso che siano due lettere
                ...
                if (!dateRegistered.matches("(19|20)[0-9][0-9]([-])(0[1-9]|1[012])([-])(0[1-9]|[12][0-9]|3[01])")) {//| significa or
                    errorMessage.setText("The date should be in the following format: YYYY-MM-DD");
                    return;
                }

                // Convert the date
                c.setDate_Registered((Date) getADate(dateRegistered));//getAdate funzione per trasformarle
                ...
                try { // Attempt to insert the information into the database
                    ...
                    MainCRUD.db.rows.updateRow();//aggiorna la riga

                    MainCRUD.db.rows.last();//si porta all'ultima riga
                    ResultSet r = MainCRUD.db.rows;//r contiene tutta la riga dei dati con resultset
                    customerID = MainCRUD.db.rows.getInt(1);//tiro fuori ID e lo metto nella variabile customerid
                    Object[] customer = {customerID, r.getString(2), r.getString(3), r.getString(4), r.getString(5), r.getString(6), r.getString(7),r.getDate(8)};//tiro su i singoli campi del record
                    MainCRUD.db.defaultTableModel.addRow(customer); // Add the row to the screen-aggiunge una riga sullo schermo
                ...
                try { // If the user clicked remove customer, delete from database and remove from table
                    ...
                    try {
                        MainCRUD.db.rows.deleteRow();//elimina la riga selezionata.
                    } catch (SQLException ex) {
                        Logger.getLogger(GUI.class.getName()).log(Level.SEVERE, null, ex);
                    }
    ...
    private class ListenForMouse extends MouseAdapter {
        ...
        public void mouseReleased(MouseEvent mouseEvent) {
            ...
            // If the mouse is released and the click was a right click//tasto destro del mouse
            if (SwingUtilities.isRightMouseButton(mouseEvent)) {
                // Create a dialog for the user to enter new data
                String value = JOptionPane.showInputDialog(null, "Enter Cell Value:");
                if (value != null) { try {//diverso da null
                    // If they entered info, update the database
                    table.setValueAt(value, table.getSelectedRow(), table.getSelectedColumn());//per andare alla cella della tabella riga+colonna
                    String updateColumn;

                    MainCRUD.db.rows.absolute(table.getSelectedRow() + 1);//posiziono sulla riga corretta
                    updateColumn = MainCRUD.db.defaultTableModel.getColumnName(table.getSelectedColumn());//updatecolum riescoa  sapere la colonna del tasto destro
                    switch (updateColumn) {
                        // if the column was date_registered, convert date update using a Date
                        case "Date_Registered":
                            sqlDateRegistered = getADate(value);//mette la stringa per la data in formato data di mysql
                            MainCRUD.db.rows.updateDate(updateColumn, (Date) sqlDateRegistered);//updatedate è il comando corretto per aggiornare la data
                            MainCRUD.db.rows.updateRow();
                            break;
                        default: // otherwise update using a String
                            MainCRUD.db.rows.updateString(updateColumn, value);//negli altri casi che non sono date aggiorna la cella
                            MainCRUD.db.rows.updateRow();
                            break;
                    }
                    } catch (SQLException ex) {
